...
parser.add_argument("--interval_in_minutes", default=60*24*7)
...
# The aggregation is partitioned by category and ordered by trans_date_trans_time.
...
agg = source_table[
    ...
    ).name(f"category_min_trans_amt_last_{interval_in_minutes}min"),
    ...
    ).name(f"category_mean_trans_amt_last_{interval_in_minutes}min"),
    # Calculate the average transaction count over the specified window.
    ...
    ).name(f"category_trans_count_last_{interval_in_minutes}min"),
    source_table.trans_date_trans_time
]

# 4. Creat Sink
sink_topic_name = f"category_trans_last_{interval_in_minutes}min"
...
connection.insert(sink_topic_name, agg)
